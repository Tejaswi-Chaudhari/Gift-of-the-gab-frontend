...
function App() {
  ...
  const loadCards = async () => {
    const result = await axios.get("https://giftofthegab.pythonanywhere.com/api/card/");
    setCards(result.data);
  }

  const loadHistorys = async () => {
    const result = await axios.get("https://giftofthegab.pythonanywhere.com/api/history/");
    setHistorys(result.data);
  }

  const loadDetails = async () => {
    const accessToken = String(localStorage.getItem('access'));
    const result = await axios.get("https://giftofthegab.pythonanywhere.com/api/current_user/",
      {
        headers: {
